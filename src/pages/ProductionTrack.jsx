...
export default function ProductionTrack() {
  ...
  // ✅ irsaliye kaydedilince waybill_logs'a da yaz
  const handleSaveWaybill = async (orderId, waybillNo) => {
    ...
    try {
      ...
      const { error } = await supabase
        .from('orders')
        .update({ current_waybill_no: waybillNo, is_waybill_issued: true })
        .eq('id', orderId);
                      ...
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex gap-3 min-w-0">
                          ...
                          <div className="min-w-0 flex flex-col justify-center gap-1">
                            <div className="font-black text-[12px] text-slate-900 tracking-tighter uppercase truncate leading-none">
                              {order.article}
                            </div>
                            ...
                            {/* ✅ RENK BİLGİSİ */}
                            {order.color && (
                              <div className="text-[8px] font-black text-slate-500 bg-slate-50 px-2 py-0.5 rounded-lg w-fit uppercase border border-slate-100">
                                {order.color}
                              </div>
                            )}
                          </div>
