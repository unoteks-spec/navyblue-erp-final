...
// Giysi türleri ve ölçü satırları (TR/EN)
const GARMENT_TYPES = {
  top: {
    label: 'Üst Giysi / Top',
    measurements: [
      ...
      { tr: 'Koltuk Altı', en: 'Armhole' },
      { tr: 'Yaka Genişliği', en: 'Neck Width' },
    ]
  },
  bottom: {
...
export default function SizeCharts() {
  ...
  // Beden grubu değişince seçimleri sıfırla
  ...
  const exportExcel = async (chart) => {
    ...
    const colCount = 3 + sizes.length + 1; // TR | EN | beden... | TOL
    const ROW_H = 18;
    ...
    // ── BİLGİ SATIRI ──
    ws.addRow([]);
    ws.getCell(2, 1).value = `Customer / Müşteri: ${chart.customer}`;
    ws.getCell(2, 1).font = { name: 'Arial', size: 9, bold: true };
    ws.getCell(2, colCount).value = new Date(chart.updated_at || chart.created_at).toLocaleDateString('tr-TR');
    ...
    ws.addRow([]);
    ws.getCell(3, 1).value = `Style / Model: ${chart.model_name}`;
    ws.getCell(3, 1).font = { name: 'Arial', size: 9, bold: true };
    ws.getCell(3, 2).value = GARMENT_TYPES[chart.garment_type]?.label || '';
    ws.getCell(3, 2).font = { name: 'Arial', size: 9, color: { argb: 'FF6B7280' } };
    ...
    chart.measurements.forEach((m, idx) => {
      ...
      row.eachCell((cell, ci) => {
        ...
        cell.alignment = { horizontal: ci <= 2 ? 'left' : 'center', vertical: 'middle' };
        cell.border = allBorder;
        if (ci === 1) cell.font = { name: 'Arial', size: 9, bold: true };
        if (ci === colCount) cell.font = { name: 'Arial', size: 9, color: { argb: 'FFEF4444' } };
      });
    });
    ...
    // ── SÜTUN GENİŞLİKLERİ ──
    ws.getColumn(1).width = 22; // TR
    ws.getColumn(2).width = 22; // EN
    for (let i = 3; i <= 2 + sizes.length; i++) {
      ws.getColumn(i).width = 8;
    }
    ws.getColumn(colCount).width = 10; // TOL
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-start justify-center p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-6xl rounded-2xl shadow-2xl my-6">
            ...
            <div className="p-6 space-y-6">
              ...
              {/* NOTLAR */}
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Notlar (opsiyonel)</label>
                <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2}
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none resize-none"
                  placeholder="Özel dikkat gerektiren notlar..."/>
              </div>
